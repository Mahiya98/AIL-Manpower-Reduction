...
// IMPORTANT: Adjust these to match your sheet's column headers
const COLS = {
  section: "Section",
  shift: "Shift",
...
function applyFilters() {
  return RAW.filter(r => {
    if (state.section !== "All" && r[COLS.section] !== state.section) return false;
    if (state.shift !== "All" && r[COLS.shift] !== state.shift) return false;
    return true;
  });
}
...
function buildFilters() {
  ...
  `).join("");
  sec.querySelectorAll("button").forEach(b => b.onclick = () => { state.section = b.dataset.sec; render(); buildFilters(); });

  const shf = document.getElementById("shiftFilters");
  shf.innerHTML = SHIFTS.map(s => {
    const count = s === "All" ? RAW.filter(r => state.section==="All" || r[COLS.section]===state.section).length
                              : RAW.filter(r => (state.section==="All" || r[COLS.section]===state.section) && r[COLS.shift]===s).length;
  ...
  shf.querySelectorAll("button").forEach(b => b.onclick = () => { state.shift = b.dataset.shf; render(); buildFilters(); });
}
...
function loadBar(pct) {
  ...
  return `<div class="flex items-center gap-2"><div class="flex-1 bg-slate-200 rounded h-1.5"><div class="${workloadColor(pct)} h-1.5 rounded" style="width:${(cap/220)*100}%"></div></div><span class="text-xs ${pct>100?'text-red-600':pct<60?'text-green-600':'text-amber-600'} font-mono">${Math.round(pct)}%</span></div>`;
}

// ===== KPI CARDS =====
function renderKPIs(data) {
  const total = data.length;
  const roles = new Set(data.map(r => r[COLS.role])).size;
  ...
  const roleMap = {};
  data.forEach(r => {
    const k = r[COLS.section] + "|" + r[COLS.role];
    if (!roleMap[k]) roleMap[k] = { section:r[COLS.section], role:r[COLS.role], hc:0, min:0 };
    roleMap[k].hc++; roleMap[k].min += Number(r[COLS.taskMin])||0;
  });
  const roles_ = Object.values(roleMap).map(x => ({...x, fte: x.min/SHIFT_BASELINE, load: (x.min/SHIFT_BASELINE)/x.hc*100}));
  ...
  document.getElementById("kpiCards").innerHTML = [
    card("Total Employees", total, "Across 5 sections"),
    ...
    card("SMS Avg Load", (sectionAvg("SMS")||0).toFixed(1)+"%", "", "text-slate-800", tag("Optimal","bg-amber-50 text-amber-700")),
    card("Rolling Avg Load", (sectionAvg("Rolling")||0).toFixed(1)+"%", "", "text-slate-800", tag("Optimal","bg-amber-50 text-amber-700")),
    card("Inventory Avg Load", (sectionAvg("Inventory")||0).toFixed(1)+"%", "", "text-green-600", tag("Optimal","bg-amber-50 text-amber-700"))
  ].join("");
...
function renderCharts(data) {
  const map = {};
  data.forEach(r => {
    const s = r[COLS.section]; if (!s) return;
    if (!map[s]) map[s] = { hc:0, min:0 };
    map[s].hc++; map[s].min += Number(r[COLS.taskMin])||0;
  });
  ...
  const loads = labels.map(l => {
    const rows = data.filter(r => r[COLS.section]===l);
    const roleMap = {};
    rows.forEach(r => { const k=r[COLS.role]; if(!roleMap[k]) roleMap[k]={hc:0,min:0}; roleMap[k].hc++; roleMap[k].min+=Number(r[COLS.taskMin])||0; });
    const arr = Object.values(roleMap).map(x => (x.min/SHIFT_BASELINE)/x.hc*100);
...
function renderRoleTable(roles_) {
  let rows = roles_.filter(r => {
    const q = state.roleSearch.toLowerCase();
    return !q || r.role.toLowerCase().includes(q) || r.section.toLowerCase().includes(q);
  });
  ...
  document.getElementById("alerts").innerHTML = [
    ...over.map(r => `<div class="flex justify-between items-center p-2 bg-red-50 rounded-lg">
        ...
        <p class="text-xs"><span class="px-1.5 py-0.5 rounded ${SECTION_COLORS[r.section]||''}">${r.section}</span> <span class="text-red-600">Overloaded</span></p></div></div>
    ...
    ...und.map(r => `<div class="flex justify-between items-center p-2 bg-cyan-50 rounded-lg">
        ...
        <div><p class="text-sm font-medium">${r.role}</p>
        <p class="text-xs"><span class="px-1.5 py-0.5 rounded ${SECTION_COLORS[r.section]||''}">${r.section}</span> <span class="text-cyan-600">Underutilised</span></p></div></div>
...
function renderEmployeeTable(data) {
  ...
  const rows = data.filter(r => !q ||
    String(r[COLS.employee]||"").toLowerCase().includes(q) ||
    String(r[COLS.role]||"").toLowerCase().includes(q) ||
    String(r[COLS.section]||"").toLowerCase().includes(q));
  ...
  document.getElementById("empTable").innerHTML = rows.map(r => {
    ...
    return `<tr class="border-t hover:bg-slate-50">
      <td class="p-2"><span class="px-2 py-0.5 rounded text-xs ${SECTION_COLORS[r[COLS.section]]||'bg-slate-100'}">${r[COLS.section]||""}</span></td>
      <td class="p-2 text-center"><span class="px-2 py-0.5 rounded-full text-xs bg-purple-50">${r[COLS.shift]||""}</span></td>
      <td class="p-2 font-medium">${r[COLS.employee]||""}</td>
      <td class="p-2 text-center text-slate-500">${r[COLS.enroll]||""}</td>
      <td class="p-2">${r[COLS.role]||""}</td>
